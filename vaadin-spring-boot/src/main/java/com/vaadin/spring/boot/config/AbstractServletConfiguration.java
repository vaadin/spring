...
public abstract class AbstractServletConfiguration implements InitializingBean {
    ...
    private void addInitParameters(
        ...
        for (Method method : methods) {
            ...
            String propertyValue = environment
                    .getProperty(getServletConfigurationParameterPrefix()
                            + name.value());
            if (propertyValue != null) {
                getLogger().info("Found servlet init parameter [{}] = [{}]",
                        name.value(), propertyValue);
                servletRegistrationBean.addInitParameter(name.value(),
                        propertyValue);
                count++;
            }
        }
        ...
        if (propertyValue != null) {
            getLogger().info("Found servlet init parameter [{}] = [{}]",
                    Constants.PARAMETER_VAADIN_RESOURCES, propertyValue);
