...
public class RequestUtilTest {
    ...
    public void testRootRequest_init_customMapping() {
        ...
        Assert.assertTrue(requestUtil.isFrameworkInternalRequest(createRequest("/bar", RequestType.INIT)));
    }

    @Test
    public void testRootRequest_other_customMapping() {
        Mockito.when(vaadinConfigurationProperties.getUrlMapping()).thenReturn("/bar/*");
        Assert.assertFalse(requestUtil.isFrameworkInternalRequest(createRequest("/bar", null)));
    }

    @Test
    public void testSubRequest_init_customMapping() {
        Mockito.when(vaadinConfigurationProperties.getUrlMapping()).thenReturn("/bar/*");
        Assert.assertFalse(requestUtil.isFrameworkInternalRequest(createRequest("/bar/foo", RequestType.INIT)));
    }
    ...
    public void testSubRequest_other_customMapping() {
        Mockito.when(vaadinConfigurationProperties.getUrlMapping()).thenReturn("/bar/*");
        Assert.assertFalse(requestUtil.isFrameworkInternalRequest(createRequest("/bar/foo", null)));
    }
    ...
    static HttpServletRequest createRequest(String pathInfo, RequestType type, Map<String, String> headers) {
